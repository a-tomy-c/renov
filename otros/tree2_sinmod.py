...
from PySide6.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, 
                               QWidget, QTreeWidget, QTreeWidgetItem, QLabel, QHBoxLayout)
from PySide6.QtCore import Qt
from PySide6.QtGui import QDragEnterEvent, QDropEvent, QIcon

class VentanaTreeWidgetDragDrop(QMainWindow):
    def __init__(self):
        super().__init__()
        self.inicializar_ui()
        
    def inicializar_ui(self):
        self.setWindowTitle("TreeWidget con Drag and Drop - PySide6")
    ...
    def agregar_archivo_al_tree(self, ruta_archivo):
        """Agrega un archivo al TreeWidget"""
        ...
        if nombre_carpeta in self.carpetas:
            ...
            # Crear nuevo item de carpeta (sin padre = top level)
            item_carpeta = QTreeWidgetItem()
            item_carpeta.setText(0, nombre_carpeta)
            item_carpeta.setIcon(0, icono_folder)
            item_carpeta.setData(0, Qt.UserRole, directorio_padre)  # Guardar ruta completa
            item_carpeta.setToolTip(0, f"Carpeta: {directorio_padre}")
            
            # Agregar al TreeWidget
            self.tree_widget.addTopLevelItem(item_carpeta)
            self.carpetas[nombre_carpeta] = item_carpeta
            
        # Crear item del archivo
        item_archivo = QTreeWidgetItem()
        item_archivo.setText(0, nombre_archivo)
        item_archivo.setIcon(0, icono_video)
        item_archivo.setData(0, Qt.UserRole, ruta_archivo)  # Guardar ruta completa
        item_archivo.setToolTip(0, f"Archivo: {ruta_archivo}")
        
        # Verificar si el archivo ya existe en esta carpeta
        archivo_existe = False
        for i in range(item_carpeta.childCount()):
            child = item_carpeta.child(i)
            if child.text(0) == nombre_archivo:
                archivo_existe = True
                break
                
        if not archivo_existe:
            # Agregar el archivo como hijo de la carpeta
            item_carpeta.addChild(item_archivo)
            
        # Expandir la carpeta para mostrar el archivo
        item_carpeta.setExpanded(True)
...
class VentanaConSeleccion(VentanaTreeWidgetDragDrop):
    def __init__(self):
        super().__init__()
        self.conectar_seleccion()
        
    def conectar_seleccion(self):
        """Conecta el evento de selección del TreeWidget"""
        self.tree_widget.itemClicked.connect(self.item_seleccionado)
        
    def item_seleccionado(self, item, column):
        """Se ejecuta cuando se selecciona un item"""
        ruta = item.data(0, Qt.UserRole)
        nombre = item.text(0)
        
        if item.parent() is None:
            # Es una carpeta (no tiene padre)
            self.label_info.setText(f"Carpeta seleccionada: {nombre}")
            print(f"Carpeta seleccionada: {ruta}")
        else:
            # Es un archivo (tiene padre)
